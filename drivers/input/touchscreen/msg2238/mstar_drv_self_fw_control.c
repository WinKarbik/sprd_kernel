...
u32 GetFirmwareVersionFor22xx(void)
{
	u32 nRegData1, nRegData2;
	u32 version_code;
	...
	version_code = (((((nRegData1 >> 8) & 0xFF) << 8) + (nRegData1 & 0xFF)) <<16 )|((((nRegData2 >> 8) & 0xFF) << 8) + (nRegData2 & 0xFF));
...
int DrvFwCtrlHandleTestIIC(void)
{
	u8 iic_buf[8];
	int try_count = 0;
	int ret = -1;
	...
	do{
		if(IicReadData(SLAVE_I2C_ID_DWI2C, &iic_buf[0], 8) >= 0)
		{
			ret = 0;
			break;
		}
...
extern int MsKeyIsExchange(void);
#endif
	...
	if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
	{
		...
		else //touch on screen
		{
			if (tInfo.nTouchKeyCode != 0)
			{
				...
				else if (tInfo.nTouchKeyCode == 4) // TOUCH_KEY_BACK
				{
#ifdef CTP_EXCHANGE_KEY
					if(MsKeyIsExchange())
					{
